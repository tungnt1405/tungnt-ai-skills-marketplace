...
function makeSuperpowersFixture() {
  ...
  writeText(
    ...
    '---\nname: using-superpowers\ndescription: upstream bootstrap\n---\n\n# Upstream bootstrap\n',
  );
...
test('syncSkills dry-run plans superpowers changes without writing', () => {
  ...
  assert.equal(result.sources[0].summary.added, 0);
  assert.equal(result.sources[0].summary.updated, 0);
  assert.equal(result.sources[0].summary.removed, 0);
  assert.equal(result.sources[0].summary.skipped, 3);
  assert.equal(fs.readFileSync(path.join(repoRoot, 'skills', 'brainstorming', 'SKILL.md'), 'utf8').includes('# Local'), true);
});

test('syncSkills apply keeps superpowers review-only and does not add files', () => {
  const repoRoot = makePackageFixture();
  const upstream = makeSuperpowersFixture();
  writeText(
    path.join(repoRoot, 'skills', 'brainstorming', 'SKILL.md'),
  ...
  assert.equal(fs.existsSync(path.join(repoRoot, 'skills', 'using-superpowers', 'SKILL.md')), false);
  assert.equal(fs.existsSync(path.join(repoRoot, 'skills', 'new-upstream-skill', 'scripts', '__pycache__', 'tool.pyc')), false);
});

test('syncSkills apply preserves stale local files for superpowers source', () => {
  const repoRoot = makePackageFixture();
  const upstream = makeSuperpowersFixture();
  writeText(
    path.join(repoRoot, 'skills', 'brainstorming', 'SKILL.md'),
  ...
  assert.equal(result.sources[0].summary.added, 0);
  assert.equal(result.sources[0].summary.removed, 0);
  assert.equal(result.sources[0].summary.skipped, 4);
  assert.equal(fs.existsSync(path.join(repoRoot, 'skills', 'new-upstream-skill')), true);
  assert.equal(fs.existsSync(path.join(repoRoot, 'skills', 'new-upstream-skill', 'stale', 'old.txt')), true);
...
test('addSyncSource supports review-only policy for forked workflow sources', () => {
  const repoRoot = makePackageFixture();
  const upstream = makeExternalSkillsRootFixture();
  ...
  assert.equal(entry.policy, 'review-only');

  const result = syncSkills({
    repoRoot,
    sourceIds: ['forked-pack'],
    apply: true,
  });

  assert.equal(result.sources[0].summary.added, 0);
  assert.equal(result.sources[0].summary.skipped, 3);
  assert.equal(fs.existsSync(path.join(repoRoot, 'skills', 'alpha', 'SKILL.md')), false);
});
...
test('sync-skills --dry-run prints deterministic source summary', () => {
  ...
  assert.equal(out.stdout().includes('Added: 0'), true);
  assert.equal(out.stdout().includes('Updated: 0'), true);
  assert.equal(out.stdout().includes('Skipped: 3'), true);
  assert.equal(fs.existsSync(path.join(repoRoot, 'skills', 'new-upstream-skill', 'SKILL.md')), false);
});
...
test('sync-skills add-source writes registry through CLI', () => {
  ...
  assert.equal(out.stdout().includes('Policy: preserve-existing'), true);
  assert.equal(loadSyncSources(repoRoot)['external-pack'].sourcePath, 'skills');
});

test('sync-skills add-source accepts review-only policy through CLI', () => {
  const repoRoot = makePackageFixture();
  const upstream = makeExternalSkillsRootFixture();
  const out = capture();
  const code = runCli(
    ['sync-skills', 'add-source', '--name', 'forked-pack', '--repo', upstream, '--policy', 'review-only'],
    { ...fakeEnv(tempDir()), TUNGNT_AI_SKILLS_SYNC_ROOT: repoRoot },
    out.io,
  );

  assert.equal(code, 0, out.stderr());
  assert.equal(out.stdout().includes('Policy: review-only'), true);
  assert.equal(loadSyncSources(repoRoot)['forked-pack'].policy, 'review-only');
});
