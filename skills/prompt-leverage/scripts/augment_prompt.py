...
TASK_KEYWORDS = {
    ...
    "skill-authoring": ["skill", "skills", "frontmatter", "trigger", "workflow", "prompt-leverage"],
}
...
def infer_intensity(prompt: str, task: str) -> str:
    ...
    if any(token in lowered for token in ["careful", "deep", "thorough", "high stakes", "production", "critical"]):
        return "Deep"
    if task in {"coding", "research", "review", "skill-authoring"}:
        return "Standard"
    return "Light"
...
def build_tool_rules(task: str) -> str:
    ...
    if task == "skill-authoring":
        return "Inspect the existing skill conventions and validation tests first. Preserve manual trigger behavior unless explicitly asked to change it."
...
def build_output_contract(task: str) -> str:
    if task == "coding":
        return "Return a concise summary, concrete changes or code, validation notes, and any remaining risks."
    if task == "research":
    ...
    if task == "skill-authoring":
        return "Return skill changes that include trigger contract, workflow compatibility notes, supporting files, and validation results."
